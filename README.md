...
The command above executes the tests for the RabbitMQ integration and builds an executable file called `nri-rabbitmq` under the `bin` directory. 
...
If you want to know more about usage of `./bin/nri-rabbitmq`, pass the `-help` parameter:
...
We encourage your contributions to improve the RabbitMQ integration! Keep in mind when you submit your pull request, you'll need to sign the CLA via the click-through using CLA-Assistant. You only have to sign the CLA one time per project.
...
## License
nri-rabbitmq is licensed under the [Apache 2.0](http://apache.org/licenses/LICENSE-2.0.txt) License.
